...
async def start(event):
  if event.is_private:
    async for usr in client.iter_participants(event.chat_id):
     ...
     await client.send_message(log_qrup, f"🗯 **𝖸𝖾𝗇𝗂 𝗂𝗍𝗂𝗋𝖺𝖿𝖼̧ı -** {ad}")
     return await event.reply(f"{ad} {startmesaj}", buttons=(
                      [
                       Button.inline("🗯 𝗂𝗍𝗂𝗋𝖺𝖿 𝖸𝖺𝗓", data="etiraf")
                      ],
                      [Button.url('📜 𝗂𝗍𝗂𝗋𝖺𝖿 𝖪𝖺𝗇𝖺𝗅ı', f'https://t.me/{kanal}')],
                      [Button.url('📣 𝖲𝗎𝗉𝗉𝗈𝗋𝗍', f'https://t.me/{support}'),
                       Button.url('👨🏻‍💻 𝖲𝖺𝗁𝗂𝖻𝗂𝗆', f'https://t.me/{sahib}')]
                    ),
                    link_preview=False)
...
async def handler(event):
    await event.edit(f"{etirafyaz}", buttons=(
                      [
                      Button.inline("🏠 𝖠𝗇𝖺 𝖲𝖺𝗒𝖿𝖺", data="start")
                      ]
                    ),
                    link_preview=False)
...
async def anonim(event):
    ...
    async for usr in client.iter_participants(event.chat_id):
     ...
     tesdiq = await yeni_etiraf.reply("Etiraf Təsdiqlənsin ?", buttons=(
                      [
                       Button.inline("✅ Kabul", data="tesdiq"
                       ),
...
async def aciq(event):
    ...
    async for usr in client.iter_participants(event.chat_id):
     ...
     tesdiq = await yeni_etiraf.reply("Etiraf Təsdiqlənsin ?", buttons=(
                      [
                       Button.inline("✅ Təsdiqlə", data="Kabul"
                       ),
                       Button.inline("🗑️ Sil", data="sil")
                      ]
                    ),
...
async def tesdiq(event):
    ...
    if tesdiq.reply_to_msg_id:
      ...
      await event.edit(f"✅ **itiraf Kabul edildi**")
